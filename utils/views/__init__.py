...
class PaginatedHelpCommand(commands.HelpCommand):
    def __init__(self):
        super().__init__(
            command_attrs={
                'help': 'Arată informații despre comenzile botului, sau despre o comandă specifică.',
                'aliases': ('comenzi', 'cmd', 'cmds', 'comandă', 'comanda', 'ajutor', 'commands', 'command')
            }
        )
    ...
    def common_command_formatting(self, embed_like, command):
        ...
        if alias:
            try:
                embed_like.add_field(name="Aliasuri", value=", ".join(alias), inline=False)
                set_aliases = True
